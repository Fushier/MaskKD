...
class Converter(nn.Module):
    def __init__(self, feat_s, feat_t):
        ...
        self.n = min(len(feat_s), len(feat_t))
        self.converters = nn.ModuleList()
        self.upsample = nn.ModuleList()
        if len(feat_s) < len(feat_t):
            feat_t = feat_t[1:]
        elif len(feat_s) > len(feat_t):
            feat_s = feat_s[1:]
        for i in range(self.n):
            self.converters.append(AAEmbed(feat_s[i].shape[1], feat_t[i].shape[1]))
        for i in range(self.n - 1, 0, -1):
            # self.upsample.append(AAEmbed(feat_t[i].shape[1], feat_t[i - 1].shape[1]))
            self.upsample.append(AAEmbed(feat_t[-1].shape[1], feat_t[i - 1].shape[1]))

    def forward(self, feat_s, feat_t):
        trans_s, trans_t, mask = [], [], []
        if len(feat_s) < len(feat_t):
            feat_t = feat_t[1:]
        elif len(feat_s) > len(feat_t):
            feat_s = feat_s[1:]
        for i in range(self.n):
            sh, th = feat_s[i].shape[2], feat_t[i].shape[2]
            if sh >= th:
                tmp_s = F.adaptive_avg_pool2d(feat_s[i], (th, th))
                trans_s.append(self.converters[i](tmp_s))
                trans_t.append(feat_t[i])
            else:
                tmp_t = F.adaptive_avg_pool2d(feat_t[i], (sh, sh))
                trans_s.append(self.converters[i](feat_s[i]))
                trans_t.append(tmp_t)

        # tmp = torch.where(trans_t[-1].sum(1, keepdim=True)<= 0, torch.tensor(0.5).cuda(), torch.tensor(1.0).cuda())
        # mask.append(tmp)
        tmp = trans_t[-1]
        mask.append(torch.where(tmp <= 0, torch.tensor(0.5).cuda(), torch.tensor(1.0).cuda()))
        for i in range(self.n - 1, 0, -1):
            h, w = trans_t[i - 1].shape[2], trans_t[i - 1].shape[3]
            # tmp = self.upsample[-i](F.interpolate(tmp, (h, w), mode='nearest'))
            tmp = self.upsample[-i](F.interpolate(trans_t[-1], (h, w), mode='nearest'))
            mask.append(torch.where(tmp <= 0, torch.tensor(0.5).cuda(), torch.tensor(1.0).cuda()))
            # tmp = F.interpolate(tmp, (h, w), mode='nearest')
            # mask.append(tmp)
        mask.reverse()
        return trans_s, trans_t, mask
